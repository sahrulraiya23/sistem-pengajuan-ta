...
class JudulTAController extends Controller
{
    ...
    public function processSubmission(Request $request, $id)
    {
        ...
        if ($request->tindakan == 'tunjuk_dosen') {
            ...
            // Sinkronkan dosen saran yang dipilih ke tabel pivot
            $pengajuan->dosenSarans()->sync($request->dosen_saran_ids);

            // Kirim notifikasi ke setiap dosen saran yang ditunjuk
            foreach ($request->dosen_saran_ids as $dosenSaranId) {
                $dosenSaran = User::find($dosenSaranId);
                if ($dosenSaran) {
